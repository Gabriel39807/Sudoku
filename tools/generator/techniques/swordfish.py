...
class Technique(BaseTechnique):
    ...
    def apply(self, board: Board, candidates: Candidates) -> TechniqueResult:
        for value in range(1, 10):
            ...
            row_cols = {r: cols for r, cols in row_cols.items() if 2 <= len(cols) <= 3}
            for rows in combinations(row_cols, 3):
            ...
            col_rows = {c: rows for c, rows in col_rows.items() if 2 <= len(rows) <= 3}
            for cols in combinations(col_rows, 3):
