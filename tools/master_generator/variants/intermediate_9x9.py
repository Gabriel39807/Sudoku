"""Intermediate 9x9 dataset generator — 1000 levels with tramo-based technique progression."""
...
TRAMO_CONFIG = [
    {"start": 1, "end": 250, "clues": 58, "label": "Tramo 1 — Singles+",
     "tech_ids": {"last_blank_cell", "full_house", "naked_single", "hidden_single"},
     "techniques": ["LastBlank", "FullHouse", "NakedSingle", "HiddenSingle"],
     "tier_max": 1, "est_minutes": 3},
    {"start": 251, "end": 500, "clues": 56, "label": "Tramo 2 — Intersections",
     "tech_ids": {"last_blank_cell", "full_house", "naked_single", "hidden_single",
                  "pointing_pair", "pointing_triple"},
     "techniques": ["LastBlank", "FullHouse", "NakedSingle", "HiddenSingle",
                    "PointingPair", "PointingTriple"],
     "tier_max": 2, "est_minutes": 4},
    {"start": 501, "end": 750, "clues": 55, "label": "Tramo 3 — Naked Pairs",
     "tech_ids": {"last_blank_cell", "full_house", "naked_single", "hidden_single",
     ...
     "techniques": ["LastBlank", "FullHouse", "NakedSingle", "HiddenSingle",
                    ...
                    "NakedPair"],
     "tier_max": 3, "est_minutes": 5},
    {"start": 751, "end": 1000, "clues": 54, "label": "Tramo 4 — Hidden Pairs & Triples",
     "tech_ids": {"last_blank_cell", "full_house", "naked_single", "hidden_single",
                  "pointing_pair", "pointing_triple", "box_line_reduction",
...
class Intermediate9x9Generator:
    """Generates intermediate 9x9 puzzles with tramo-based technique limitation."""
...
def _pick_symmetry(idx: int) -> str:
    seed = (idx * 7 + 13) % 100
    return "mirror" if seed < 30 else "rotational"


def generate_intermediate_dataset(output_dir: str, checkpoint_path: str,
                                  seed: int = 42) -> List[Dict]:
    os.makedirs(output_dir, exist_ok=True)
    gen = Intermediate9x9Generator(seed=seed)
    ...
    for tramo in TRAMO_CONFIG:
        for idx in range(tramo["start"], tramo["end"] + 1):
            board_id = f"intermediate_{idx:04d}"
            if board_id in generated_ids:
                print(f"  {board_id}: already generated, skipping")
                continue

            sym = _pick_symmetry(idx)
            print(f"  {board_id}: clues={tramo['clues']} techs={len(tramo['tech_ids'])} {sym} ...", end="")
            start = time.time()
