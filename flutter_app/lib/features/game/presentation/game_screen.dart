...
class _GameScreenState extends ConsumerState<GameScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<GameState>(gameProvider, (previous, next) {
      if (previous?.status != GameStatus.won && next.status == GameStatus.won) {
        _showVictoryDialog();
      } else if (previous?.status != GameStatus.lost && next.status == GameStatus.lost) {
        _showDefeatDialog();
      }
    });
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Column(
                              ...
                              children: [
                                const Text('DIFICULTAD', style: TextStyle(fontSize: 10, color: Colors.white54)),
                                Text(
                                  state.difficulty,
                            ...
                            Row(
                              children: [
                                Column(
                                  ...
                                  children: [
                                    const Text('ERRORES', style: TextStyle(fontSize: 10, color: Colors.white54)),
                                    Text(
                                      '${state.errors} / 3',
                                      style: TextStyle(
                                        fontWeight: FontWeight.bold,
                                        fontSize: 16,
                                        color: state.errors > 0 ? Colors.redAccent : Colors.white,
                                      ),
                                    ),
                                  ],
                                ),
  ...
  Future<void> _showVictoryDialog() async {
    ...
    return showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: Theme.of(context).cardTheme.color,
        title: const Text('VICTORY', style: TextStyle(color: Colors.greenAccent, fontWeight: FontWeight.bold, fontSize: 24)),
        content: Column(
  ...
  Future<void> _showDefeatDialog() async {
    return showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: Theme.of(context).cardTheme.color,
        title: const Text('DEFEAT', style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.bold, fontSize: 24)),
